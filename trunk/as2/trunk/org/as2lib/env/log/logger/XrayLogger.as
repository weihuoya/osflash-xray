 ...
 * method from Keith Peter's Debug Panel.  Now extended to use Keith's Debug to dispatch
 * an event that allows other methods of delivery with debug messags.
 * 
 ...
 * @author Simon Wacker
 * @author John Grden
 * @see <a href="http://www.osflash.org/xray#usage_8.11.2006">Xray Logging</a>
 ...
 */
//class org.as2lib.env.log.logger.XrayLogger extends AbstractLogger implements Logger 
class org.as2lib.env.log.logger.XrayLogger extends AbstractLogger implements Logger
{	
	 ...
	 * traced objects
	 */
	public function XrayLogger(traceObject:Boolean, recursionDepth:Number, indentation:Number) 
	{
	 ...
	 */
	public function setLevel(levelName:String):Void 
	{
		// convert to uppercase
		levelName = levelName.toUpperCase();
		
		// get the LogLevel
		var logLevel:LogLevel = AbstractLogLevel.forName(levelName);
		...
		// resolve the numeric prepresentation
		this.levelAsNumber = this.level.toNumber();
		
		// when level is changed via XrayLogger, we update Debug so it knows what to pass along to the interface
		Debug.setLevel(this.levelAsNumber);
		Debug.setLevelName(levelName);
	}
	 ...
	 */
	public function log(msg_0, msg_1, className:String):Void 
	{
		if (this.traceObject) 
		{
			for(var i:Number=0;i<2;i++)
			{
				var message:Object = i == 0 ? msg_0 : msg_1;
				
				// if msg_1 wasn't passed, break out
				if(i == 1 && msg_1 == undefined) break;
				
				// if msg_0, append className, methodCalled
				if(i == 0 && className.length > 0)
				{
					message = className + "\n" + message;
				}
				...
				{
					// added line break for traces
					//Debug.trace(message.toString()+"\n");
					Debug.trace(message.toString(), className);
				} 
				else 
				{
					Debug.traceObject(message, this.recursionDepth, this.indentation, className);
				}
			}
		} 
		else 
	 ...
	 * @see #isDebugEnabled
	 */
	public function debug(msg_0, msg_1, caller:Object):Void 
	{
		if (isDebugEnabled()) 
		{
			// get calling classPackage and method names
			var className:String = "";
			if(caller != undefined) className = ClassPath.getClass(caller, true, false);			
			log(msg_0, msg_1, className);
		}
	}
	 ...
	 * @see #isInfoEnabled
	 */
	public function info(msg_0, msg_1, caller:Object):Void {
		if (isInfoEnabled()) {
			// get calling classPackage and method names
			var className:String = "";
			if(caller != undefined) className = ClassPath.getClass(caller, true, false);			
	 ...
	 */
	public function warning(msg_0, msg_1, caller:Object):Void {
		if (isWarningEnabled()) {
			// get calling classPackage and method names
			var className:String = "";
			if(caller != undefined) className = ClassPath.getClass(caller, true, false);			
	 ...
	 * @see #isErrorEnabled
	 */
	public function error(msg_0, msg_1, caller:Object):Void {
		if (isErrorEnabled()) {
			// get calling classPackage and method names
			var className:String = "";
			if(caller != undefined) className = ClassPath.getClass(caller, true, false);			
	 ...
	 * @see #isFatalEnabled
	 */
	public function fatal(msg_0, msg_1, caller:Object):Void {
		if (isFatalEnabled()) {
			// get calling classPackage and method names
			var className:String = "";
			if(caller != undefined) className = ClassPath.getClass(caller, true, false);			
